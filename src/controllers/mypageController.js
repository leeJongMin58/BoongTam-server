...
export const getUserReview = async (req, res) => {
    try {
        ...
        if (!tab || (tab !== 'store' && tab !== 'goods')) {
            ...
            if (reviewId) {
                // 특정 리뷰 검색
                reviews = await mypageModel.findReviewById(userId, tab, reviewId);
                if (!reviews) {
                    return res.status(404).json({
                        ...errorCode[404],
                ...
                // 전체 리뷰 검색
                reviews = await mypageModel.findReviewsByTab(userId, tab);
            }
...
export const updateUserReview = async (req, res) => {
    try{
        ...
        if (!tab || (tab !== 'store' && tab !== 'goods')) {
            ...
            const result = await mypageModel.updateReviewById(userId, tab, reviewId, review_text, rating);
...
export const deleteUserReview = async (req, res) => {
    try{
        ...
        if (!tab || (tab !== 'store' && tab !== 'goods')) {
            ...
            const result = await mypageModel.deleteReviewById(userId, tab, reviewId);
